...
- `test/*.test.mjs` — `node --test`. Pure suites for the libs (including upstream-converted regression cases), plus an assembly suite that mounts real `Context` + real `SessionStore`/`ToolRuntime`/`Commands` from the installed peers (pinned `0.1.7-alpha.2` devDependencies; the seam was originally verified against the `0.1.2-rc.1` peers) with a structurally complete fake agent.

## Seam conclusions (verified against the `0.1.2-rc.1` peers; re-checked on the `0.1.3-alpha.1` checkout 2026-09-04 and against the `0.1.6-alpha.2` line 2026-09-18)
...
- **Session append is two-argument** — the installed peers (pinned `0.1.7-alpha.2`; originally the `0.1.2-rc.1` line) have no append-envelope option, and the two-argument form typechecks against rc.2 and newer builds. `translate/fix` is declared via `types.d.ts` declaration merging and is log-only (counts/flags, never payloads). The append goes through the adaptive gate (`KNOWN_SESSION_EVENT_TYPES` membership first, then the `ignorable`-in-source probe): hosts that know the type append plainly, hosts with the `ignorable` envelope append with the marker, and envelope-less hosts (`0.1.0-rc.6`–`0.1.1-rc.2`, `0.1.2-rc.1`, which fails closed on unknown types at read) get no append — a repair never pollutes the session log, and the repair outcome never depends on the audit.
- **checkJs boundaries**: `defineTool`'s generic inference cannot be reconciled across the two peer type copies in this JS-form repo, so the `fix_json` options object and a few boundary params carry documented `any` casts; runtime behavior is covered by the assembly tests, and `types.d.ts` documents the public surface. `types.d.ts` anchors the `translate/fix` event merge at **both** `@deepseek-ai/dsh-session/types` and the bare `@deepseek-ai/dsh-session` specifier: the checkout face resolves the session service through the bare entry, so a single-subpath merge silently stops applying there (found while turning `check` into a real checkout face). The repo's `check` gate must stay green.
