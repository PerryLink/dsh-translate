...
- `test/*.test.mjs` — `node --test`. Pure suites for the libs (including upstream-converted regression cases), plus an assembly suite that mounts real `Context` + real `SessionStore`/`ToolRuntime`/`Commands` from the 0.1.0-rc.8 peers with a structurally complete fake agent.

## Seam conclusions (verified against 0.1.0-rc.8)
...
- **Session append is two-argument** — the pinned rc.8 peers have no append-envelope option, and the two-argument form typechecks against rc.8 and newer builds. `translate/fix` is declared via `types.d.ts` declaration merging and is log-only (counts/flags, never payloads).
