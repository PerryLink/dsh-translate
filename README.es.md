...
- En hosts que ni conocen `translate/fix` ni exponen el envoltorio `ignorable` en `session.append` — la línea publicada `0.1.0-rc.6`–`0.1.1-rc.2` y `0.1.2-alpha.3`, que falla cerrado ante tipos de evento desconocidos al leer — la puerta adaptativa omite el append de auditoría para que el registro de sesión nunca se contamine; el espejo de auditoría se pierde en esos hosts hasta que el tipo de evento se registre.
